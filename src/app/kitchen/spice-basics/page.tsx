...
const spiceData = {
  'zh-CN': {
    chinese: [
        ...
        { id: 'sichuanPepper', name: '花椒', features: '辛香带麻，层次丰富', preprocessing: '干用或炝油；适配川菜、牛羊肉', compatibleFood: '川菜、牛肉、羊肉、豆制品', pairingSuggestion: '辣椒、孜然、生姜', efficacy: '性温，归脾、胃、肾经；温中止痛、杀虫止痒、除湿；可辅助缓解寒湿泄泻、胃寒腹痛', contraindications: '阴虚火旺者（牙龈肿痛、便秘）忌用；孕妇慎用（过量可能刺激子宫）；热性皮肤病（湿疹、痤疮）患者慎用' },
        { id: 'cassia', name: '桂皮', features: '辛香微甜，香味持久', preprocessing: '整段用，敲碎易出味；适配卤味、甜品', compatibleFood: '猪肉、牛肉、卤味、甜品', pairingSuggestion: '八角、丁香、草果', efficacy: '性大热，归肾、脾、心、肝经；补火助阳、散寒止痛、温经通脉；可辅助缓解肾阳不足、腰膝冷痛', contraindications: '阴虚火旺者（潮热盗汗、口干咽燥）忌用；孕妇忌用（易致胎动不安）；高血压、上火者（口舌生疮）慎用' },
        { id: 'bayLeaf', name: '香叶', features: '淡辛香，提味不抢镜', preprocessing: '直接用，1-2片即可；适配各类炖菜', compatibleFood: '所有肉类、卤菜、炖菜', pairingSuggestion: '八角、桂皮、花椒', efficacy: '性温，归肺、脾、肝经；祛风除湿、行气止痛、散寒；可辅助缓解风湿痹痛、脘腹胀痛', contraindications: '阴虚火旺者慎用；气虚体弱、易疲劳者不宜过量' },
        { id: 'fennelSeed', name: '小茴香', features: '甘香温和，带茴香特有甜香', preprocessing: '整粒或磨粉；炒制后香味更浓', compatibleFood: '猪肉、鱼肉、豆制品、面食', pairingSuggestion: '花椒、八角、酱油', efficacy: '性温，归肝、肾、脾、胃经；散寒止痛、理气和胃；可辅助缓解寒疝腹痛、胃寒食少', contraindications: '阴虚火旺者（眼红、口干）忌用；过敏体质者慎用' },
        { id: 'caoguo', name: '草果', features: '浓烈辛辣，祛腥力强', preprocessing: '去籽用，拍裂出味；适配牛羊肉', compatibleFood: '牛肉、羊肉、内脏', pairingSuggestion: '白芷、生姜、陈皮', efficacy: '性热，归脾、胃经；燥湿温中、截疟除痰；可辅助缓解寒湿内阻、脘腹胀痛、疟疾', contraindications: '阴虚火旺者（口干、便秘）忌用；孕妇慎用；热性咳嗽（黄痰、咽痛）患者忌用' },
        { id: 'chenpi', name: '陈皮', features: '辛苦带果香，解腻增香', preprocessing: '泡软刮白瓤；适配炖肉、甜品', compatibleFood: '炖肉、甜品', pairingSuggestion: '生姜、红枣、红豆', efficacy: '性温，归肺、脾经；理气健脾、燥湿化痰；可辅助缓解脾胃气滞（腹胀、食欲不振）、湿痰咳嗽', contraindications: '阴虚燥咳者（干咳无痰、咽干口燥）慎用；舌红少津、内有实热（发烧、便秘）者不宜过量' },
        { id: 'cuminChinese', name: '孜然（中式）', features: '香辣浓郁，异域感强', preprocessing: '炒香磨粉；适配烧烤、羊肉', compatibleFood: '烧烤、羊肉', pairingSuggestion: '辣椒、羊肉、洋葱', efficacy: '性热，归肝、胃经；散寒止痛、理气和胃；可辅助缓解胃寒腹痛、消化不良、风湿痹痛', contraindications: '阴虚火旺者（口干、长痘）忌用；夏季炎热时慎用；便秘、痔疮患者慎用' },
        { id: 'perilla', name: '紫苏', features: '辛香清新，祛腥解腻', preprocessing: '鲜用或干泡；适配鱼类、螃蟹', compatibleFood: '鱼类、螃蟹、田螺', pairingSuggestion: '辣椒、大蒜、生姜', efficacy: '性温，归肺、脾经；解表散寒、行气和胃、安胎；可辅助缓解风寒感冒、妊娠呕吐、鱼蟹中毒', contraindications: '风热感冒者（发热、黄痰）忌用；气虚表虚者（易出汗、乏力）慎用' },
        { id: 'mint', name: '薄荷', features: '辛香清凉，提神解腻', preprocessing: '鲜用或泡水；适配饮品、甜肴', compatibleFood: '饮品、甜肴', pairingSuggestion: '柠檬、青柠、巧克力', efficacy: '性凉，归肺、肝经；疏散风热、清利头目、利咽；可辅助缓解风热感冒、头痛、咽喉肿痛', contraindications: '脾胃虚寒者（腹泻、怕冷）忌用；孕妇慎用；气虚自汗者（一动就出汗）慎用' },
        { id: 'gardenia', name: '黄栀子', features: '微苦，天然黄色色素', preprocessing: '敲裂泡水调色；适配卤菜、炖菜', compatibleFood: '卤菜、炖菜', pairingSuggestion: '八角、桂皮、酱油', efficacy: '性寒，归心、肝、肺、胃、三焦经；泻火除烦、清热利湿、凉血解毒；可辅助缓解热病心烦、湿热黄疸', contraindications: '脾胃虚寒者（腹痛、腹泻）忌用；阳虚体质者（怕冷、手脚冰凉）慎用' },
        { id: 'monkFruit', name: '罗汉果', features: '味甜，天然甜味剂', preprocessing: '敲开煮水；适配卤菜、甜汤', compatibleFood: '卤菜、甜汤', pairingSuggestion: '雪梨、百合、卤料包', efficacy: '性凉，归肺、大肠经；清热润肺、利咽开音、滑肠通便；可辅助缓解肺热燥咳、咽痛失声、便秘', contraindications: '脾胃虚寒者（腹泻、便溏）忌用；风寒感冒咳嗽者（白痰、怕冷）慎用' },
        { id: 'licorice', name: '甘草', features: '味甜，调和卤味口感', preprocessing: '切片用；适配卤水、腌制品', compatibleFood: '卤水、腌制品', pairingSuggestion: '八角、桂皮、陈皮', efficacy: '性平，归心、肺、脾、胃经；补脾益气、清热解毒、调和诸药；可辅助缓解脾胃虚弱、心悸气短', contraindications: '湿盛胀满者（腹胀、舌苔厚腻）忌用；长期过量食用可能导致水钠潴留（水肿、血压升高）；糖尿病患者慎用' },
        { id: 'angelica', name: '白芷', features: '苦香辛凉，祛腥力强', preprocessing: '切片用；适配羊肉、鱼类', compatibleFood: '羊肉、鱼类', pairingSuggestion: '草果、生姜、花椒', efficacy: '性温，归胃、大肠、肺经；解表散寒、祛风止痛、宣通鼻窍；可辅助缓解风寒感冒、头痛、鼻塞', contraindications: '阴虚血热者（口干、牙龈出血）忌用；阴虚火旺者慎用；有光敏反应者（易晒黑、长斑）慎用' },
        { id: 'cardamomBlack', name: '砂仁', features: '浓烈芳香，增香解腻', preprocessing: '整粒用；适配荤菜、豆制品', compatibleFood: '荤菜、豆制品', pairingSuggestion: '草果、白芷、丁香', efficacy: '性温，归脾、胃、肾经；化湿开胃、温脾止泻、理气安胎；可辅助缓解湿浊中阻、脘腹胀满、妊娠呕吐', contraindications: '阴虚火旺者（口干、便秘）忌用；实热积滞者（腹痛、口臭）慎用' },
    ],
    european: [
      { id: 'rosemary', name: '迷迭香', features: '木质香+松针感，微苦', preprocessing: '取嫩枝烤用；适配羊肉、土豆', compatibleFood: '羊肉、牛肉、鸡肉、土豆', pairingSuggestion: '橄榄油、大蒜、黑胡椒', efficacy: '性温，归肺、脾经；辅助提神醒脑、缓解疲劳；外用可辅助改善皮肤循环（需稀释）', contraindications: '高血压患者慎用（可能轻微升高血压）；孕妇慎用（传统认为可能刺激子宫）；过敏体质者（接触后皮肤发红）忌用' },
      { id: 'thyme', name: '百里香', features: '柔和花香+柠檬味', preprocessing: '取叶炖用；适配鸡汤、鱼肉', compatibleFood: '鸡肉、鱼肉、炖菜、奶油酱', pairingSuggestion: '洋葱、胡萝卜、奶油', efficacy: '性温，归肺、脾经；辅助解表散寒、止咳化痰；可缓解轻微风寒感冒、咳嗽痰多', contraindications: '阴虚火旺者（口干、咽痛）慎用；过敏体质者（接触后皮肤瘙痒）忌用' },
      { id: 'basil', name: '罗勒', features: '甜香带薄荷感，清新浓郁', preprocessing: '鲜用出锅前加；适配番茄、披萨', compatibleFood: '意大利菜、番茄、鸡肉、披萨', pairingSuggestion: '番茄、大蒜、橄榄油', efficacy: '性温，归肺、胃经；辅助理气和胃、缓解消化不良；可缓解轻微腹胀、食欲不振', contraindications: '脾胃虚寒者（腹泻、怕冷）慎用；过量可能导致失眠（含少量兴奋成分）' },
    ...
    southeastAsian: [
      { id: 'lemongrass', name: '香茅', features: '浓烈柠檬香+草本涩感', preprocessing: '切段拍裂；适配冬阴功、河粉', compatibleFood: '海鲜、鸡肉、椰浆料理', pairingSuggestion: '南姜、柠檬叶、椰浆', efficacy: '性温，归肺、胃经；辅助祛风除湿、散寒止痛、开胃消食；可缓解轻微风湿痛、胃寒腹胀', contraindications: '阴虚火旺者（口干、便秘）慎用；皮肤敏感者（接触后可能刺激）忌用' },
    ...
    middleEastern: [
      { id: 'cuminMiddleEast', name: '孜然籽', features: '辛辣带坚果香，清新', preprocessing: '炒香研磨；适配烤肉、鹰嘴豆', compatibleFood: '中东烤肉、鹰嘴豆、咖喱', pairingSuggestion: '芫荽籽、小豆蔻、柠檬汁', efficacy: '性热，归肝、胃经；辅助散寒止痛、理气和胃；可缓解轻微胃寒腹痛、消化不良', contraindications: '阴虚火旺者（口干、便秘）忌用；夏季慎用；痔疮患者慎用' },
      ...
      { id: 'turmeric', name: '姜黄', features: '微苦姜香，天然黄色色素', preprocessing: '粉用或鲜磨；适配咖喱、米饭', compatibleFood: '咖喱、米饭、汤羹', pairingSuggestion: '黑胡椒、生姜、椰奶', efficacy: '性温，归肝、脾经；辅助行气活血、通经止痛；现代研究提示可辅助改善循环（需遵医嘱）', contraindications: '孕妇忌用（有活血作用，可能致胎动不安）；有出血倾向者（如凝血功能障碍）慎用；胆结石患者慎用' },
  ...
  'en': {
    chinese: [
        ...
        { id: 'fennelSeed', name: 'Fennel Seed', features: 'Sweet, warm, licorice-like flavor', preprocessing: 'Use whole or ground; toast to enhance', compatibleFood: 'Pork, Fish, Beans, Breads', pairingSuggestion: 'Sichuan Pepper, Star Anise, Soy Sauce', efficacy: 'Warm; benefits liver, kidney, spleen, stomach; dispels cold, stops pain, regulates qi, harmonizes stomach.', contraindications: 'Avoid if yin-deficient with fire (red eyes, dry mouth); caution for those with allergies.' },
    ...
    european: [
      { id: 'rosemary', name: 'Rosemary', features: 'Woody, pine-like, slightly bitter', preprocessing: 'Use tender sprigs for roasting; adapts to lamb, potatoes', compatibleFood: 'Lamb, Beef, Chicken, Potatoes', pairingSuggestion: 'Olive Oil, Garlic, Black Pepper', efficacy: '(Reference) Warm; benefits lung, spleen; may help improve focus, relieve fatigue; topical use may improve skin circulation (diluted).', contraindications: 'Caution for hypertension (may slightly raise blood pressure); caution for pregnant women; avoid if allergic (skin redness).' },
      { id: 'thyme', name: 'Thyme', features: 'Mild, floral & lemony', preprocessing: 'Use leaves in stews; adapts to chicken soup, fish', compatibleFood: 'Chicken, Fish, Stews, Cream Sauces', pairingSuggestion: 'Onion, Carrot, Cream', efficacy: '(Reference) Warm; benefits lung, spleen; may help release exterior, stop cough, resolve phlegm for mild wind-cold conditions.', contraindications: 'Caution for yin-deficient fire (dry mouth, sore throat); avoid if allergic (skin itching).' },
      { id: 'basil', name: 'Basil', features: 'Sweet, peppery, fresh, rich', preprocessing: 'Add fresh leaves at the end; adapts to tomato, pizza', compatibleFood: 'Italian, Tomato, Chicken, Pizza', pairingSuggestion: 'Tomato, Garlic, Olive Oil', efficacy: '(Reference) Warm; benefits lung, stomach; may aid digestion, relieve bloating.', contraindications: 'Caution for those with cold-deficient spleen/stomach (diarrhea); excess may cause sleeplessness (contains mild stimulants).' },
    ...
    middleEastern: [
      ...
      { id: 'turmeric', name: 'Turmeric', features: 'Earthy, slightly bitter, yellow dye', preprocessing: 'Use powder or grate fresh; adapts to curries, rice', compatibleFood: 'Curries, Rice, Soups', pairingSuggestion: 'Black Pepper, Ginger, Coconut Milk', efficacy: '(Reference) Warm; benefits liver, spleen; may help move qi and blood, relieve pain. Modern studies suggest circulatory benefits (consult doctor).', contraindications: 'Forbidden for pregnant women (activates blood); caution for bleeding tendencies; caution for gallstones.' },
...
const TipCard: React.FC<{ icon: React.ElementType; title: string; content: string; }> = ({ icon: Icon, title, content }) => (
    <div className="relative p-5 rounded-lg bg-card/70 border border-border/20 shadow-sm backdrop-blur-sm overflow-hidden">
        <div className="absolute top-3 right-3 text-primary/10">
            <Icon className="w-10 h-10" />
        </div>
        <div className="relative z-10">
            <h4 className="flex items-center gap-2 font-semibold text-foreground/90 mb-2">
