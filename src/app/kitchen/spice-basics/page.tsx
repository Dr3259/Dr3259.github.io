...
const translations = {
  'zh-CN': {
    ...
    tips: {
        ...
        principleContent: '浓烈香料（丁香、肉豆蔻、卡宴辣椒）过量易掩盖食材本味：丁香每次≤3粒，肉豆蔻≤1/4颗，卡宴辣椒从1/4茶匙开始尝试。热性香料（桂皮、草果、孜然、姜黄）夏季需减量，避免上火（例：1kg肉配草果不超过1颗）。',
        prepTitle: '预处理黄金法则',
        prepContent: '干香料（八角、桂皮、香叶）：温水浸泡10分钟（去浮尘、激发香味），或小火微炒至微黄（勿炒糊，否则发苦）。新鲜香料（罗勒、薄荷、紫苏）：烹饪前洗净沥干，出锅前加入（避免久煮导致香味流失、颜色变黑）。带籽香料（草果、豆蔻）：去籽使用（籽味苦，影响口感）；坚硬香料（南姜、桂皮）拍裂/切块，方便出味。',
        pairingTitle: '搭配逻辑：“功能互补”更出味',
        pairingContent: '重腥膻食材（羊肉、内脏）：祛腥（白芷、南姜）+ 增香（八角、迷迭香）+ 解腻（陈皮、欧芹），三者协同效果最佳。例：炖羊肉 = 白芷（祛膻）+ 八角（增香）+ 陈皮（解腻）。清淡食材（鱼肉、蔬菜）：选清新香料（紫苏、柠檬叶、百里香），避免用浓郁香料（桂皮、丁香）压味。例：清蒸鱼 = 紫苏叶（祛腥）+ 柠檬片（提鲜）。卤菜/炖菜“基础公式”：以10斤水为例，八角5颗+桂皮2段+香叶5片+小茴香1小把（祛腥增香基底），可根据食材加草果（牛肉）、黄栀子（调色）。',
        storageTitle: '储存与替换技巧',
        storageContent: '储存：干香料密封避光，放干燥阴凉处（保质期6-12个月）；新鲜草本（罗勒、香茅）用湿纸包裹冷藏，或插入清水（每周换水，可存1-2周）。新鲜vs干制替换：新鲜香料与干制香料风味差异大，替换比例约为 3:1（3份新鲜=1份干制），干制香料早加入（耐煮），新鲜香料出锅前加（保清新）。',
    },
    fusion: {
      title: '跨菜系融合案例 (实操参考)',
      case1Title: '中式卤水 + 东南亚香料',
      case1Desc: '基础卤水（八角+桂皮+香叶）中加入1段香茅+2片柠檬叶，卤出的鸡肉自带清新柠檬香，解腻又独特。',
  ...
  'en': {
    ...
    tips: {
        ...
        principleContent: 'Strong spices (cloves, nutmeg, cayenne) can overpower dishes: use ≤3 cloves, ≤1/4 nutmeg, start with 1/4 tsp cayenne. Use warming spices (cassia, caoguo, cumin, turmeric) sparingly in summer (e.g., max 1 caoguo per 1kg meat).',
        prepTitle: 'Golden Rules for Preprocessing',
        prepContent: 'Dry spices (star anise, cassia): Soak in warm water for 10 mins (cleans & awakens flavor) or lightly toast (don\'t burn). Fresh herbs (basil, mint): Wash, pat dry, add at the end of cooking. Seeded spices (caoguo, cardamom): Deseed to reduce bitterness. Hard spices (galangal, cassia): Crush or slice to release flavor.',
        pairingTitle: 'Pairing Logic: Complementary Functions',
        pairingContent: 'Strong-flavored meats (lamb, offal): Use a "de-fishing (angelica root/galangal) + aromatizing (star anise/rosemary) + degreasing (chenpi/parsley)" combo. E.g., Lamb stew = Angelica + Star Anise + Chenpi. Delicate foods (fish, vegetables): Use fresh herbs (perilla, lime leaf, thyme); avoid overpowering spices (cloves). E.g., Steamed fish = Perilla + Lemon.',
        storageTitle: 'Storage & Substitution',
        storageContent: 'Storage: Store dry spices in a cool, dark, airtight place (6-12 months). Keep fresh herbs in a damp paper towel in the fridge or in water (change weekly, lasts 1-2 weeks). Fresh vs. Dry: The ratio is roughly 3:1 (fresh:dry). Add dry spices early; add fresh herbs at the end.',
    },
    fusion: {
...
const spiceData = {
  'zh-CN': {
    ...
    southeastAsian: [
      ...
      { name: '高良姜', features: '辛辣微甜，带樟木香气', preprocessing: '可替代部分生姜', pairing: '羊肉串、螃蟹、牛肉', usage: '印尼沙爹、新加坡辣椒蟹', suggestions: '椰浆、鱼露' },
      { name: '香兰叶', features: '清甜椰香，自带绿色', preprocessing: '多用作包裹食材蒸煮（如香兰叶包鸡），或榨汁调色', pairing: '米饭、蛋糕、椰浆汤', usage: '香兰叶包鸡、椰浆饭', suggestions: '米饭、蛋糕、椰浆' },
    ],
    middleEastern: [
  ...
  'en': {
    chinese: [
      ...
      { name: 'Dried Tangerine Peel', features: 'Bitter & fruity, cuts grease', preprocessing: 'Soften in warm water (5 mins), scrape off white pith (reduces bitterness)', pairing: 'Duck, beef, ribs', usage: '1. Lamb Stew: 2-3 pieces with red dates to cut grease.<br>2. Red Bean Soup: Cook with red beans for fragrance.', suggestions: 'Ginger, red dates, red beans' },
    ...
    southeastAsian: [
      ...
      { name: 'Galangal (alt)', features: 'Spicy and sweet with a piney aroma', preprocessing: 'Can substitute for some ginger', pairing: 'Lamb skewers, crab, beef', usage: 'Indonesian satay, Singapore chili crab.', suggestions: 'Coconut milk, fish sauce' },
      { name: 'Pandan Leaf', features: 'Sweet coconut-like aroma, provides green color', preprocessing: 'Often used to wrap food for steaming/cooking, or juiced for color', pairing: 'Rice, cakes, coconut milk soups', usage: 'Pandan-wrapped chicken, coconut rice.', suggestions: 'Rice, cakes, coconut milk' },
    ],
    middleEastern: [
        { name: 'Cumin Seed (Mid-East)', features: 'Pungent & nutty, fresher than powder', preprocessing: 'Dry-toast for 1-2 mins (no oil), then grind for intense flavor', pairing: 'Middle Eastern BBQ, hummus, curries', usage: '1. Lamb Skewers: Ground cumin with cardamom and lemon juice.<br>2. Hummus: Sprinkle on top with olive oil.', suggestions: 'Coriander seed, cardamom, lemon juice' },
