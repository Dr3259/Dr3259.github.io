...
interface QuickDraftModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (data: { content: string; date: Date; images?: string[] }) => void;
  translations: {
    title: string;
    placeholder: string;
    saveButton: string;
          ...
          <Tabs defaultValue="content">
            ...
            <TabsContent value="content">
              <Textarea
                ...
                className="min-h-[220px] text-base leading-relaxed resize-none border-2 border-border/50 focus:border-amber-700 focus:ring-4 focus:ring-amber-700/15 outline-none rounded-xl p-4 shadow-sm"
              ...
              <div className="grid grid-cols-3 gap-3 mt-3">
                {images.map((img, idx) => (
                  <div key={idx} className="border rounded-lg p-2 flex flex-col gap-2">
                    ...
                    <Button variant="outline" onClick={() => setImages(prev => prev.filter((_, i) => i !== idx))} disabled={isOcrLoading} className="py-2">
                      {isZh ? '删除图片' : 'Remove'}
                    </Button>
                  </div>
                ))}
            ...
            <TabsContent value="actions">
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="outline" onClick={() => {
                  ...
                  if (urls.length === 0) {
                    const msg = isZh ? '未发现链接' : 'No links found';
                    toast({ title: msg, duration: 2000 });
                    return;
                  }
                  const now = new Date(); const dateKey = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')}`; const slotKey = `${String(now.getHours()).padStart(2, '0')}:00 - ${String(now.getHours() + 1).padStart(2, '0')}:00`;
                  Array.from(new Set(urls)).forEach(url => addShareLink(dateKey, slotKey, { id: Date.now().toString(), url, title: url, category: null }));
                  const msg = isZh ? '链接已保存到当前时段' : 'Links saved to current slot';
                  toast({ title: msg, duration: 2000 });
                }} className="py-2">
                ...
                <Button variant="outline" onClick={() => {
                  ...
                  const now = new Date(); const dateKey = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')}`; const slotKey = `${String(now.getHours()).padStart(2, '0')}:00 - ${String(now.getHours() + 1).padStart(2, '0')}:00`;
                  addTodo(dateKey, slotKey, { text, completed: false, category: null, deadline: null, importance: null });
                  const msg = isZh ? '已转换为待办' : 'Converted to to-do';
                  toast({ title: msg, duration: 2000 });
                }} className="py-2">
                  {isZh ? '转为待办' : 'To To-do'}
                </Button>
                <Button variant="outline" onClick={() => {
                  const text = content.trim(); if (!text) return;
                  const now = new Date(); const dateKey = `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')}`; const slotKey = `${String(now.getHours()).padStart(2, '0')}:00 - ${String(now.getHours() + 1).padStart(2, '0')}:00`;
                  addReflection(dateKey, slotKey, { text, category: '', timestamp: new Date().toISOString() });
                  const msg = isZh ? '已转换为灵感' : 'Converted to reflection';
                  toast({ title: msg, duration: 2000 });
                }} className="py-2">
